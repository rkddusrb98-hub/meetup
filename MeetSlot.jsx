...
function computeRecos(days, durMin, participants, options) {
  ...
  days.forEach((d, di) => {
    ...
    for (const s of SLOTS) {
      ...
      if (!t) continue;
      cands.push({ dayKey: d.key, di, start: s, end: s + durMin, ...t });
    }
  });
...
function recoReason(p) {
  switch (p.tier) {
    case 2: return "일부 참석자 확인이 필요해요";
    case 3: return "회의실만 조정하면 가능해요";
    case 4: return "필수 참석자 확인이 필요해요";
    case 1:
    default: return "가장 빠른 시간이에요";
  }
}
